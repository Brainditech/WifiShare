...
exports.getLocalIPSync = getLocalIPSync;
 ...
 */
function getPrimaryOutboundIP(timeoutMs = 500) {
 ...
 */
function getLocalIPFromInterfaces() {
    const nets = (0, os_1.networkInterfaces)();
    const candidates = [];
    for (const [name, interfaces] of Object.entries(nets)) {
        if (!interfaces)
            continue;
        if (hasVirtualName(name))
            continue;
        for (const net of interfaces) {
            if (net.family !== 'IPv4' || net.internal)
                continue;
            // Skip MACs from known virtual-adapter vendors
            if (isVirtualMac(net.mac))
                continue;
 ...
 */
function isAddressOnRealInterface(ip) {
    const nets = (0, os_1.networkInterfaces)();
    for (const [name, interfaces] of Object.entries(nets)) {
        if (!interfaces)
            continue;
